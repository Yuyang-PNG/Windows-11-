...
class GPUInfo:
    def __init__(self, name: str, gpu_type: str, brand: str, memory_total: int = 0):
        self.name = name
        self.type = gpu_type
        self.brand = brand
...
class GPUManager:
    ...
    def get_gpu_info(self, force_refresh: bool = False) -> List[Dict]:
    ...
    def _detect_concurrent(self) -> List[Dict]:
        gpus = []
        detected_names = set()
        results = []
        threads = []

        def detect_with_timeout(detector: GPUDetector, results_list: List):
            try:
    ...
    def get_gpu_recommendation(self, process_name: str) -> Dict:
