...
class RLPriorityAgent:
    ...
    ACTION_SPACE = [0, 1, 2, 3, 4]  # IDLE, BELOW_NORMAL, NORMAL, ABOVE_NORMAL, HIGH
    ...
    def get_state(self, cpu: float, memory: float, category: str, hour: int, is_gaming: bool) -> tuple:
        """将连续状态转换为离散状态"""
        return (
            min(int(cpu / 20), 4),  # 5个区间 [0-4]
            min(int(memory / 20), 4),
            hash(category) % 10,  # 10个类别
            hour,  # 24小时
    ...
    def calculate_reward(self, process: dict, system_metrics: dict) -> float:
        ...
        # 系统高负载时降低非关键进程 = 正奖励
            ...
            reward = 5.0
        # 非游戏进程被错误提升 = 负奖励
        elif process.get('category') not in ['gaming', 'ai', 'design'] and process.get('priority') == 'high':
            reward = -3.0
        ...
        if memory > 90 and process.get('category') not in ['system', 'security']:
            reward -= 3.0
