...
try:
    ...
    WIDGET_BP_AVAILABLE = True
except ImportError:
    WIDGET_BP_AVAILABLE = False
...
class ProcessPriorityAPI:
    def __init__(self, port=5000, debug=False):
        ...
        # 注册 widget blueprint
        if WIDGET_BP_AVAILABLE:
            self.app.register_blueprint(widget_bp)
    ...
    def _register_routes(self):
        ...
        @self.app.route('/api/history/<process_name>', methods=['GET'])
        def get_process_history(process_name):
            try:
                days = request.args.get('days', 7, type=int)
                if self.history_manager:
                    history = self.history_manager.get_process_history(process_name, days)
        ...
        @self.app.route('/api/history/<process_name>/changes', methods=['GET'])
        def get_priority_changes(process_name):
            try:
                days = request.args.get('days', 7, type=int)
                if self.history_manager:
                    changes = self.history_manager.get_priority_changes(process_name, days)
        ...
        @self.app.route('/api/history/<process_name>/stats', methods=['GET'])
        def get_process_stats(process_name):
            try:
                days = request.args.get('days', 7, type=int)
                if self.history_manager:
                    stats = self.history_manager.get_process_avg_stats(process_name, days)
                    return jsonify({'success': True, 'data': stats})
                else:
                    return jsonify({'success': False, 'error': 'History manager not available'}), 500
            except Exception as e:
        ...
        @self.app.route('/api/ml/rl/train', methods=['POST'])
        def train_rl_model():
            try:
                if not self.rl_trainer:
                    return jsonify({'success': False, 'error': 'RL Trainer not available'}), 500
                
                data = request.get_json() or {}
                episodes = data.get('episodes', 100)
                result = self.rl_trainer.train_from_history(episodes)
                return jsonify({'success': result['status'] == 'success', 'data': result})
            except Exception as e:
                return jsonify({'success': False, 'error': str(e)}), 500
        
        @self.app.route('/api/ml/rl/info', methods=['GET'])
        def get_rl_info():
            try:
                if not self.rl_trainer:
                    return jsonify({'success': False, 'error': 'RL Trainer not available'}), 500
                
                info = self.rl_trainer.get_model_info()
                return jsonify({'success': True, 'data': info})
            except Exception as e:
                return jsonify({'success': False, 'error': str(e)}), 500
        
        @self.app.route('/api/config/watch/status', methods=['GET'])
        def get_config_watch_status():
            try:
                from process_priority_manager import CONFIG_WATCHER
                return jsonify({
                    'success': True,
                    'data': CONFIG_WATCHER.get_watch_status() if CONFIG_WATCHER else {'running': False}
                })
            except Exception as e:
                return jsonify({'success': False, 'error': str(e)}), 500
        
        @self.app.route('/api/config/watch/start', methods=['POST'])
        def start_config_watch():
            try:
                from process_priority_manager import CONFIG_WATCHER
                if CONFIG_WATCHER and not CONFIG_WATCHER.running:
                    CONFIG_WATCHER.start()
                return jsonify({'success': True})
            except Exception as e:
                return jsonify({'success': False, 'error': str(e)}), 500
        
        @self.app.route('/api/config/watch/stop', methods=['POST'])
        def stop_config_watch():
            try:
                from process_priority_manager import CONFIG_WATCHER
                if CONFIG_WATCHER and CONFIG_WATCHER.running:
                    CONFIG_WATCHER.stop()
                return jsonify({'success': True})
            except Exception as e:
                return jsonify({'success': False, 'error': str(e)}), 500
